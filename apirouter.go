...
func newAPIRouter(app *appContext, userRealIP bool) apiMux {
	...
	//mux.HandleFunc("/status", app.status)
	mux.Route("/coinsupply", func(r chi.Router) {
		r.Route("/", func(rd chi.Router) {
			rd.Get("/", app.getCoinSupply)
		})
	})
	...
	// 		ProjectPath: "github.com/HcashOrg/hcexplorer",
	// 		Intro:       "hcexplorer HTTP router directory",
	// 	}))
	// 	return
	// }
...
func (mux *apiMux) ListenAndServeProto(listen, proto string) {
	...
	if proto == "https" {
		go http.ListenAndServeTLS(listen, "hcexplorer.cert", "hcexplorer.key", mux)
	}

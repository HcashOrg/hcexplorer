...
func (b *BlockData) ToBlockExplorerSummary() apitypes.BlockExplorerBasic {
	...
	return apitypes.BlockExplorerBasic{
		...
		AiVoters:               b.Header.AiVoters,
		Revocations:            b.Header.Revocations,
		AiRevocations:          b.Header.AiRevocations,
		FreshStake:             b.Header.FreshStake,
		AiFreshStake:           b.Header.AiFreshStake,
		StakeDiff:              b.Header.SBits,
		AiStakeDiff:            b.Header.AiSBits,
...
// it into the BlockDataBasic and StakeInfoExtended and hcdataapi types.
...
func (t *Collector) CollectBlockInfo(hash *chainhash.Hash) (*apitypes.BlockDataBasic,
	...
	*apitypes.BlockExplorerExtraInfo, *wire.MsgBlock, error) {
	...
	coinSupply, err := t.hcdChainSvr.GetCoinSupply()
	if err != nil {
		log.Error("GetCoinSupply failed: ", err)
	}
	nbSubsidy, err := t.hcdChainSvr.GetBlockSubsidy(int64(msgBlock.Header.Height)+1, 5)
	if err != nil {
	...
	blockHeaderResults, err := t.hcdChainSvr.GetBlockHeaderVerbose(hash)
	if err != nil {
		return nil, nil, nil, nil, nil, err
	}
...
func (t *Collector) CollectHash(hash *chainhash.Hash) (*BlockData, *wire.MsgBlock, error) {
	...
	numConn, err := t.hcdChainSvr.GetConnectionCount()
	if err != nil {
...
func (t *Collector) Collect() (*BlockData, *wire.MsgBlock, error) {
	...
	go func() {
		bestBlockHash, err := t.hcdChainSvr.GetBestBlockHash()
	...
	// Stake difficulty
	stakeDiff, err := t.hcdChainSvr.GetStakeDifficulty()
	if err != nil {
		return nil, nil, err
	}
